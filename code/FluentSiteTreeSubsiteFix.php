 ...
 * @author lekoala
 */
class FluentSiteTreeSubsiteFix extends FluentExtension
{
    ...
    public function updateRelativeLink(&$base, &$action)
    {
        ...
        if (!$this->owner->exists()) {
            $base = Controller::join_links($base,
                    '?'.Fluent::config()->query_param.'='.urlencode($locale));
            return;
        }
    ...
    public function LocaleLink($locale)
    {
        ...
        if (!$this->owner->exists()) {
            ...
            return Controller::join_links($url,
                    '?'.Fluent::config()->query_param.'='.urlencode($locale));
        }
