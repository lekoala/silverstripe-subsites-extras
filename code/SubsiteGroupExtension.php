...
class SubsiteGroupExtension extends DataExtension implements PermissionProvider
{
    ...
    public function requireDefaultRecords()
    {
        ...
        if (isset($groupFields['SubsiteID'])) {
            ...
            DB::getConn()->renameField('Group', 'SubsiteID',
                '_obsolete_SubsiteID');
    ...
    public function updateCMSFields(FieldList $fields)
    {
        if ($this->owner->canEdit()) {
            // i18n tab
            $fields->findOrMakeTab('Root.Subsites',
            ...
            if (isset($subsiteMap[0])) {
                $fields->addFieldToTab("Root.Subsites",
                    new OptionsetField("AccessAllSubsites",
                    _t('GroupSubsites.ACCESSRADIOTITLE',
                        'Give this group access to'),
                    array(
                ...
                unset($subsiteMap[0]);
                $fields->addFieldToTab("Root.Subsites",
                    new CheckboxSetField("Subsites", "", $subsiteMap));
            } else {
                if (sizeof($subsiteMap) <= 1) {
                    $fields->addFieldToTab("Root.Subsites",
                        new ReadonlyField("SubsitesHuman",
                        _t('GroupSubsites.ACCESSRADIOTITLE',
                            'Give this group access to'), reset($subsiteMap)));
                } else {
                    $fields->addFieldToTab("Root.Subsites",
                        new CheckboxSetField("Subsites",
                        _t('GroupSubsites.ACCESSRADIOTITLE',
                            'Give this group access to'), $subsiteMap));
                }
            }
        }
    }
    ...
    public function alternateTreeTitle()
    {
        if ($this->owner->AccessAllSubsites) {
            ...
            $subsites = Convert::raw2xml(implode(", ",
                        $this->owner->Subsites()->column('Title')));
    ...
    public function augmentSQL(SQLQuery &$query)
    {
        ...
        if (!$query->filtersOnID()) {
            ...
            foreach ($query->getFrom() as $item) {
                if ((is_array($item) && strpos($item['table'], 'Group_Subsites')
                    !== false) || (!is_array($item) && strpos($item,
            ...
            if (!$hasGroupSubsites) {
                if ($subsiteID) {
                    $query->addLeftJoin("Group_Subsites",
                        "\"Group_Subsites\".\"GroupID\"
            ...
            if ($hasGroupSubsites && $subsiteID && isset($select[0]) && !$select[0]
                == 'COUNT(*)') {
                $query->orderby = "\"AccessAllSubsites\" DESC".($query->orderby ? ', '
                            : '').$query->orderby;
            }
        }
    }
    ...
    public function providePermissions()
    {
        return array(
            'SECURITY_SUBSITE_GROUP' => array(
                'name' => _t('GroupSubsites.MANAGE_SUBSITES',
                    'Manage subsites for groups'),
