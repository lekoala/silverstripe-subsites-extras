...
class SubsiteSecurityAutocompleter extends Controller {
	...
	public function autocomplete() {
		...
		if(!singleton('Member')->hasDatabaseField($fieldName)  || $fieldName == 'Password') return;
		...
		$matches = DataObject::get("Member","`$fieldName` LIKE '" . Convert::raw2sql($fieldVal) . "%' ".$where, $orderBy = '`'.$fieldName.'` ASC', ' INNER JOIN `Group_Members` ON `Group_Members`.`MemberID` = `Member`.`ID`');
		if($matches->count() > 0) {
			$result .= "<ul>";
