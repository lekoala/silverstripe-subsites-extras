 ...
 * @author lekoala
 */
class DataObjectSubsites extends DataExtension {
	...
	function augmentSQL(SQLQuery &$query, DataQuery &$dataQuery = null) {
		...
		if ($dataQuery->getQueryParam('Subsite.filter') === false)
			return;
		if (get_class(Controller::curr()) == 'Security')
			return;
	...
	function updateCMSFields(FieldList $fields) {
		...
		if (Subsite::currentSubsiteID()) {
			...
			if (!$field) {
				$fields->addFieldToTab('Root.Subsite', new DropdownField('SubsiteID', 'Subsite', $subsitesMap));
			}
		}
	}
